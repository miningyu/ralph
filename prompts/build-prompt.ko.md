...
구현 전, 각 task의 `context` 필드가 있다면 읽으세요. planner가 scope analysis에서 추출한 Why / Current state / Gotcha를 담고 있으며, 숨은 제약과 결정사항에 대해 권위 있는 정보로 취급하세요. Acceptance는 여전히 통과/실패의 기준이며, context는 그것을 보조하는 상황 인식 정보입니다.
